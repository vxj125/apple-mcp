...
tell application "Mail"
    return its version
                ...
                throw new Error('Mail app is running but cannot access mailboxes. Please check permissions and configuration.');
            }
        }
    } catch (error) {
        console.error('Mail access check failed:', error);
        throw new Error(`Cannot access Mail app. Please make sure Mail is running and properly configured. Error: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
tell application "Mail"
    ...
    return resultList
            ...
            console.log('AppleScript unread result:', asResult);
            ...
            if (asResult && asResult.includes('subject') && asResult.includes('sender')) {
            ...
            console.log('AppleScript approach failed:', asError);
...
tell application "Mail"
    ...
    return unreadInfo
                ...
                for (const account of accounts) {
                    try {
                        ...
                        try {
                            ...
                            console.log(`Account ${accountName} has ${accountMailboxes.length} mailboxes`);
                            ...
                            for (const mailbox of accountMailboxes) {
                                try {
                                    ...
                                    let unreadMessages;
                                    try {
                                        unreadMessages = mailbox.messages.whose({ readStatus: false })();
                                        console.log(`Found ${unreadMessages.length} unread in ${boxName}`);
                                        ...
                                        console.log(`Error getting unread for ${boxName}: ${unreadError}`);
                                    }
                                } catch (boxError) {
                            ...
                            console.log(`Error getting mailboxes for ${accountName}: ${mbError}`);
                        }
        ...
        throw new Error(`Error accessing mail: ${error instanceof Error ? error.message : String(error)}`);
    }
}

async function searchMails(searchTerm: string, limit: number = 10): Promise<EmailMessage[]> {
    try {
        if (!await checkMailAccess()) {
            return [];
        }
...
tell application "Mail"
    ...
    return resultList
            ...
            const asResult = await runAppleScript(script);
            console.log('AppleScript search result:', asResult);
            ...
            console.log('AppleScript search method failed:', asError);
            // Continue to JXA approach
        }

        // JXA approach as fallback
        const searchResults: EmailMessage[] = await run((searchTerm: string, limit: number) => {
            const Mail = Application('Mail');
            const results = [];
                ...
                for (const mailbox of mailboxes) {
                    try {
                        ...
                        try {
                            ...
                            console.log(`Found ${messages.length} matching messages in ${mailbox.name()}`);
                        } catch (queryError) {
                            console.log(`Error querying messages in ${mailbox.name()}:`, queryError);
                            continue;
                        }
                        ...
                        for (let i = 0; i < count; i++) {
                            try {
                                ...
                                results.push({
                                    ...
                                    sender: msg.sender(),
                                    dateSent: msg.dateSent().toString(),
                                    content: msg.content() ? msg.content().substring(0, 500) : "[No content]", // Limit content length
        ...
        throw new Error(`Error searching mail: ${error instanceof Error ? error.message : String(error)}`);
    }
}

async function sendMail(to: string, subject: string, body: string, cc?: string, bcc?: string): Promise<string> {
    try {
        if (!await checkMailAccess()) {
...
`;
        ...
        try {
            ...
            console.log('AppleScript send result:', result);
            if (result === 'success') {
                return `Email sent to ${to} with subject "${subject}"`;
            } else {
                throw new Error('AppleScript did not confirm success');
            }
        } catch (asError) {
            console.error('Error in AppleScript send:', asError);
            
            // Try alternative approach with JXA
            console.log('Attempting send with JXA approach...');
            const jxaResult = await run((to, subject, body, cc, bcc) => {
                try {
                    const Mail = Application('Mail');
            ...
            console.log('JXA send result:', jxaResult);
        ...
        throw new Error(`Error sending mail: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
if application "Mail" is not running then
    ...
    delay 2
        ...
        throw new Error(`Error getting mailboxes: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
tell application "Mail"
    ...
    return acctNames
        ...
        throw new Error(`Error getting mail accounts: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
tell application "Mail"
    ...
    return boxNames
        ...
        throw new Error(`Error getting mailboxes for account ${accountName}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
